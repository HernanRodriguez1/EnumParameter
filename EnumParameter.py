...
def procesar_url(url, lista_negra, args, urls_validas, urls_imprimidas, archivo_salida):
    ...
    if parsed.netloc == args.dominio and parsed.query:
        ...
        if all(extension not in url for extension in lista_negra) and any(query_parseado.values()):
            query_modificada = {k: [''] for k in query_parseado.keys()}
            url_modificada = urlunparse((parsed.scheme, parsed.netloc, parsed.path, '', '&'.join([f"{k}={v[0]}" for k, v in query_modificada.items()]), ''))
            if args.verificar:
...
def principal():
    ...
    if args.subsitios or args.subsitios == "True":
    ...
    if args.excluir:
        print(f"\u001b[31m[!] Las URL que contienen estas extensiones se excluirán de los resultados: {lista_negra}\u001b[0m\n")
        args.excluir = None
    else:
